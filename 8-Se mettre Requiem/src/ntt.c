...
void ntt(ull* arr, int n, ull prime, ull root, ull mu, int is_inverse) {
    ...
    main_loop:
        ...
        outer_butterfly_loop:
            ...
            inner_butterfly_loop:
                ...
                ull u = arr[i_plus_j];
                ull v = barrett_reduce(mult(arr[ijdivlen], w), prime, mu);
                arr[i_plus_j] = barrett_reduce(add(u, v), prime, mu);
                arr[ijdivlen] = barrett_reduce((add(sub(u, v), prime)), prime, mu);
    ...
    if (is_inverse) {
        ...
        scale_loop:
            ...
            arr[i] = barrett_reduce(mult(arr[i], n_inv), prime, mu);
            i = add(i, 1);
            goto scale_loop;
        scale_end:;
    }
...
void multiply_ntt(ll res[], int num1[], int len1, int num2[], int len2) {
    ...
    prime_loop:
        ...
        if (EQUAL(mod((sub(PRIMES[i], 1)), n), 0)) {
    ...
    pointwise_mult_loop:
        ...
        ntt_c[i] = barrett_reduce((mult(ntt_a[i], ntt_b[i])), prime, mu);
        i = add(i, 1);
